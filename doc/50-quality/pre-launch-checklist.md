...
- [ ] Redis/Valkey 已开启 AOF 持久化。
- [ ] Postgres 数据库已配置自动备份脚本（建议每日凌晨）。
- [ ] OSS Bucket 权限已设置为公共读（针对音频/封面）且私有写。
- [ ] Nginx 已配置 Gzip 压缩及 HSTS 安全头。

## 2. 功能验证 (Functionality)
- [ ] 注册与登录：支持密码登录，JWT Token 续装期正常。
- [ ] 投稿闭环：上传 -> 待审 -> 后台通过 -> 前台展示 链路通畅。
- [ ] 搜索与筛选：支持多标签、风格、原创性联合筛选。
- [ ] 开放 API：`/music/open/api/v1/meta` 返回正常，API Key 校验有效。
- [ ] Webhook 推送：异步重试机制已激活，日志可追溯。

## 3. 性能与压力测试 (Performance)
- [ ] 单接口压测：核心 `GET /songs/{id}` QPS 需达 500+。
- [ ] 全链路模拟：支持 1000 人同时在线播放不出现卡顿。
- [ ] 慢 SQL 治理：`p6spy`监控中无超过 500ms 的执行记录。
